...
export default function About() {
    ...
    return (
        <section id="about" className="py-16 md:py-24 relative overflow-hidden w-full">
            <div className="container mx-auto px-4 sm:px-6 md:px-12">

                {/* Top Grid: Text + Photo — stacks on mobile, side-by-side on lg */}
                <div className="grid lg:grid-cols-12 gap-8 md:gap-12 lg:gap-16 items-center mb-16 md:mb-24 max-w-7xl mx-auto">

                    {/* Left: Text Content — full width on mobile, order-2 so photo appears first on mobile */}
                    <div className="lg:col-span-7 flex flex-col justify-center text-center lg:text-left order-2 lg:order-1">
                        <div className="flex flex-col text-[2rem] leading-[1.05] sm:text-5xl md:text-6xl lg:text-[3.8rem] xl:text-[4.5rem] font-black tracking-tighter mb-6 md:mb-8 font-heading uppercase text-foreground md:leading-[0.9]">
                        ...
                        <div className="text-base md:text-lg lg:text-xl text-muted-foreground leading-relaxed space-y-4 md:space-y-6 font-sans">
                            <p>
                            ...
                            <div className="space-y-4 pt-6 mt-6 border-t border-border">
                                <h4 className="text-xl md:text-2xl font-bold text-foreground font-heading tracking-tight uppercase">My Objectives</h4>
                                <p>
                                ...
                                <div className="py-4">
                                    <h5 className="text-xs font-mono text-muted-foreground uppercase tracking-widest mb-4">I aim to:</h5>
                                    <ul className="space-y-3">
                                        {[
                                            ...
                                            >
                                                <span className="text-primary mt-1 font-mono text-sm leading-none shrink-0">►</span>
                                                <span>{goal}</span>
                                            </motion.li>
                                        ))}
                                    </ul>
                    ...
                    {/* Right: Profile Photo — full width on mobile, shorter height, shows first */}
                    <div className="lg:col-span-5 w-full h-[280px] sm:h-[380px] md:h-[480px] lg:h-[700px] relative order-1 lg:order-2 group overflow-hidden mx-auto max-w-sm sm:max-w-none">
                        {/* Curtain Reveal Mask */}
                        <motion.div
                            initial={{ height: "100%" }}
                ...
                {/* Bottom Grid: Skills bar + Stats — stacks on mobile */}
                <div className="grid md:grid-cols-2 gap-8 md:gap-12 items-center">
                    ...
                    <motion.div
                        ...
                        className="relative p-6 md:p-8 border border-border bg-card"
                    >
                        <div className="absolute top-0 right-0 p-4 opacity-[0.03]">
                            <Dna size={160} />
                        </div>
                        <h3 className="text-xl md:text-2xl font-bold mb-4 flex items-center gap-2 font-heading uppercase tracking-widest">
                            <Play className="text-primary shrink-0" fill="currentColor" size={14} /> Research to Reality
                        </h3>
                        <p className="text-muted-foreground mb-6 md:mb-8 font-mono text-sm leading-relaxed">
                            ...
                            {[
                                ...
                                <div key={idx}>
                                    <div className="flex justify-between text-xs mb-2 text-muted-foreground font-mono uppercase tracking-widest">
                                        <span className="pr-2 truncate">{skill.name}</span>
                                        <span className="text-primary shrink-0">{skill.progress}</span>
                                    </div>
                    ...
                    {/* Stats Grid — 2 cols always, suitable on mobile */}
                    <div className="grid grid-cols-2 gap-3 sm:gap-4">
                        {stats.map((stat, idx) => (
                            ...
                            >
                                <Card className="border-border bg-card hover:border-primary transition-colors duration-300 group rounded-none h-full">
                                    <CardContent className="p-4 md:p-6 flex flex-col items-center text-center gap-3">
                                        <div className="p-3 md:p-4 bg-muted group-hover:bg-primary/10 transition-colors duration-300">
                                            {stat.icon}
                                        </div>
                                        <div>
                                            <div className="text-3xl md:text-4xl font-black text-foreground font-heading">{stat.value}</div>
                                            <div className="text-[10px] md:text-xs text-primary mt-1 md:mt-2 font-mono uppercase tracking-widest leading-tight">{stat.label}</div>
                                        </div>
                                    </CardContent>
                                </Card>
                            </motion.div>
